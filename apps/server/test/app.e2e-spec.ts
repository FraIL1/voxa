...
describe('Voxa: критический поток (e2e)', () => {
  ...
  afterAll(async () => {
    socket?.disconnect();
    /* Даём разрывам осесть до закрытия приложения: обработчик разрыва
       ходит в базу, и на медленной машине он успевал сработать уже после
       того, как Prisma закрылась. Тесты при этом все зелёные, а падал
       весь набор. Сам шлюз такую гонку тоже переживает, это второй рубеж. */
